...
if not defined APP_NAMESPACE set "APP_NAMESPACE=de-ai-08"

set "SECRET_JSON_FILE=%TEMP%\de-ai-08-rds-%RANDOM%.json"
set "DB_ENV_FILE=%TEMP%\de-ai-08-rds-%RANDOM%.env"
...
kubectl label secret rds-secret ^
  --namespace "%APP_NAMESPACE%" ^
  app.kubernetes.io/part-of=de-ai-08 ^
  --overwrite
if errorlevel 1 goto :error
